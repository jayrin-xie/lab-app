...
def test_sheet_group():
  ...
  sheet_group = SheetGroup(columns, rows, table_vals)
  ...
  assert np.array_equal(sheet_group.table_vals[0], table_vals)
  ...
  sheet_group.add_new_sheet(new_table_vals)
  
  assert len(sheet_group.table_vals) == 2
  assert np.array_equal(sheet_group.table_vals[1], new_table_vals)
